...
db_path = '/home/aiudabot/AIUDA_PACKAGES/postal_goal_ws/src/postal_goal_ros/python_script/aiuda_distribution/barangay_residents.db'
...
def get_qrcode_database(name_list):
    try:
        ...
        for name in name_list:
            cursor.execute(
                "SELECT barcode FROM Residents WHERE name IN ('{0}');".format(name))
...
def get_selected_name(qrcode_list):
    try:
        name_list = []
        sqliteConnection = sqlite3.connect(
            db_path)
        ...
        for qr_code in qrcode_list:
            cursor.execute(
                "SELECT name FROM Residents WHERE barcode IN ('{0}');".format(qr_code))
...
def get_status_database(name_list):
    try:
        status_list_sorted = []
        sqliteConnection = sqlite3.connect(
            db_path)
        ...
        for name in name_list:
            cursor.execute(
                "SELECT status FROM Residents WHERE name IN ('{0}');".format(name))
            status_result = cursor.fetchall()[0]
            status_list_sorted.append(status_result)
        cursor.close()

        # print(qrcode_list_sorted)
        return status_list_sorted
...
def postal_codes_with_rfid(rfid_list):
    ...
    for rfid in rfid_list:
        cursor.execute(
            "SELECT location FROM Residents WHERE barcode IN ('{0}');".format(rfid))
...
def ayuda_slot_assignment_algo(sorted_postal_code, qr_code_list):
    ...
    for postal_code in sorted_postal_code:
        ...
        for qr_code in qr_code_list:
            ...
            cursor.execute(
                "SELECT location FROM Residents WHERE barcode IN ('{0}');".format(qr_code))
            location_result = cursor.fetchall()[0]
            qr_code_address = [int(s) for s in re.split(
                '; |, |\*|\n|-|  | ', location_result) if s.isdigit()][0]

            cursor.execute(
                "SELECT cabinet_num FROM Residents WHERE barcode IN ('{0}');".format(qr_code))
            check_assigned = int(cursor.fetchall()[0])
            if qr_code_address == postal_code and check_assigned == 0:
                ...
                if (num % 2) == 0:
                    ...
                    num_left += 1
                print(qr_code, ayuda_slot_value)
                sql_update_query = """Update Residents set cabinet_num = {0}  where barcode = '{1}' """.format(
                    ayuda_slot_value, qr_code)
...
def update_status_delivery(qr_code_list):
    status_value = 'Delivering'
    sqliteConnection = sqlite3.connect(
        db_path)
    sqliteConnection.row_factory = lambda cursor, row: row[0]
    cursor = sqliteConnection.cursor()
    for qr_code in qr_code_list:
        sql_update_query = """Update Residents set status = '{0}'  where barcode = '{1}' """.format(
            status_value, qr_code)
        cursor.execute(sql_update_query)
    sqliteConnection.commit()
    cursor.close()


def cancel_status_delivery():
    status_value = 'Cancelled'
    sqliteConnection = sqlite3.connect(
        db_path)
    sqliteConnection.row_factory = lambda cursor, row: row[0]
    cursor = sqliteConnection.cursor()
    sql_update_query = """Update Residents set status = '{0}' where cabinet_num > 0 """.format(
        status_value)
    cursor.execute(sql_update_query)
    sqliteConnection.commit()
    cursor.close()

def arriving_update(slot):
    
    status_value = 'Arriving'
    ...
    sql_update_query = """Update Residents set status = '{0}'  where cabinet_num = '{1}' """.format(
        status_value, slot)
    cursor.execute(sql_update_query)
    sqliteConnection.commit()
    cursor.close()

def arrived_update(slot):
...
def done_or_fail_update(slot, status_var):
    ...
    # Update Slot Number
    sql_update_query = """Update Residents set cabinet_num = '{0}'  where cabinet_num = '{1}' """.format(
        str(0), slot)
    cursor.execute(sql_update_query)
    sqliteConnection.commit()
...
def get_info_cabinet_num(cabinet_num):
    ...
    cursor = sqliteConnection.cursor()
    cursor.execute(
        "SELECT location, contact, name, ayuda, barcode FROM Residents WHERE cabinet_num IN ('{0}');".format(cabinet_num))
...
#Get  Resident details for sms 
def get_info_sms(qrcode):
    try:
        sqliteConnection = sqlite3.connect(db_path)
        cursor = sqliteConnection.cursor()
        ...
        cursor.execute(
                "SELECT location,contact,name,cabinet_num FROM Residents WHERE barcode IN ('{0}');".format(qrcode))
        info_result = cursor.fetchall()
        print(info_result)
...
def find_qr_code(slot_number):

    sqliteConnection = sqlite3.connect(db_path)
    sqliteConnection.row_factory = lambda cursor, row: row[0]
    cursor = sqliteConnection.cursor()
    print(slot_number)
    cursor.execute("SELECT barcode FROM Residents WHERE cabinet_num IN ('{0}');".format(slot_number))
    qr_code_to_be_scanned  = cursor.fetchall()[0]
    print(qr_code_to_be_scanned)
    print('Here is the QR-Code need to be Scanned',qr_code_to_be_scanned)
    return qr_code_to_be_scanned



# info_result = get_info_sms('Resident-001')
# print(info_result[0][0])
