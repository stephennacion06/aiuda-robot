...
for device in device_list:
    if(device.vid != None or device.pid != None):
        ...
        if ( device.vid == gpsVid ) and ( device.pid == gpsPid ):
            port = device.device
...
def arrived_delivery_sms(telephone_num,name,address,cabinet):
    ...
    msg=ser.readline()
    print(msg)
    cmd = "Hello {0}, this is AI-UDA Robot Delivery. You are allocated to Ayuda Cabinet No.{1} and I already arrived in your location: {2}. Please bring your QR-code for receiving the Ayuda.\r".format(name,cabinet,address)
    ser.write(cmd.encode())
    ser.write(serial.to_bytes([0x1A]))
    msg=ser.readline()
    print(msg)
